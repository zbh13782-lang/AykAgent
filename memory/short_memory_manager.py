...
class ShortMemoryManager:
	def __init__(self) -> None:
		...
		self.compact_turns = max(50, settings.short_memory_compact_turns)#对话轮次数量到达这个阈值，自动触发压缩
		self.compact_chars = max(50000, settings.short_memory_compact_chars)#字符数量到达，自动触发压缩
		self.keep_turns = max(5, settings.short_memory_compact_keep_turns)#保留最近5条数据不压缩，保持语义
	...
	async def append_event(self, thread_id: str, role: str, content: str) -> None:
		...
		await self._redis.rpush(self._events_key(thread_id), json.dumps(payload, ensure_ascii=False))
	...
	async def maybe_compact(self, thread_id: str, saver: Any, reason: str = "auto") -> CompactResult | None:
		if not self.enabled:
			return None
		await self.ainit()
	...
	async def force_compact(self, thread_id: str, saver: Any, reason: str = "manual") -> CompactResult | None:
		if not self.enabled:
			return None
		await self.ainit()
		...
		raw_events = await self._redis.lrange(events_key, compressed_until, target_end - 1)
		if not raw_events:
			return None
	...
	async def _should_compact(self, thread_id: str) -> bool:
		...
		compressed_until = int(await self._redis.hget(meta_key, "compressed_until") or 0)
